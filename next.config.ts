...
class FunctionSpaceSourceResolverPlugin {
  apply(resolver: {
    ...
    resolver.getHook("beforeResolve").tapAsync(
      "FunctionSpaceSourceResolverPlugin",
      (request, resolveContext, callback) => {
        if (!request.request?.endsWith(".js") || !request.path?.includes("vendor/fs_trading_sdk/packages")) {
          callback();
          return;
        }
...
const nextConfig: NextConfig = {
  ...
  transpilePackages: ["@functionspace/core", "@functionspace/react", "@functionspace/ui"],
  webpack(config) {
